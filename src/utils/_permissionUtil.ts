...
export const PermissionsArray = Object.keys(PermissionsObj) as IPermissions[] &
  [string, ...string[]];
export const PermissionsSelection = PermissionsArray.map((key) => ({
  label: PermissionsObj[key as keyof typeof PermissionsObj],
  value: key,
}));

export const CheckPermissions = (
  Permissions: IPermissions[],
  accountPermissions: IPermissions[],
  strict: boolean = true
) => {
  let valid = false;

  if (!strict)
    valid = Permissions.some((role) =>
      accountPermissions.find((r) => r === role)
    );
  else
    valid = Permissions.every((role) =>
      accountPermissions.find((r) => r === role)
    );
  return valid;
};
